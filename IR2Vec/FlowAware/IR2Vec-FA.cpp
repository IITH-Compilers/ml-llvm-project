...
void IR2Vec_FA::initializeVectors() {
...
void IR2Vec_FA::print(SmallVector<double, DIM> vec) {
  for (auto i : vec)
    errs() << i << " ";
}
...
SmallVector<double, DIM> IR2Vec_FA::getValue(std::string key) {
...
BranchProbabilityInfo *IR2Vec_FA::getBPI(Function *F) {
...
std::string IR2Vec_FA::splitAndPipeFunctionName(std::string s) {
  std::regex igt("<.*?>");
  std::regex sp(" ");
  std::regex un("^_+");
  std::regex lu("_+$");
...
bool IR2Vec_FA::runOnModule(Module &M) {
  collectData();
  initializeVectors();
  ...
  for (auto &f : M) {
    if (!f.isDeclaration()) {
      ...
      if (level == 'f') {
        ...
        if (c2v == 'y') {
          std::string pipeFunc =
              IR2Vec_FA::splitAndPipeFunctionName(std::string(demangledName));
          res += pipeFunc + "\t";
        } else {
...
IR2Vec_FA::func2Vec(Function &F, SmallVector<Function *, 15> &funcStack,
...
char IR2Vec_FA::ID = 0;
static RegisterPass<IR2Vec_FA> X("IR2Vec_FA", "IR2Vec_FA");

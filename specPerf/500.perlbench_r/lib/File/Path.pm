...
sub _rmtree {
    ...
    foreach $root (@$paths) {
        ...
        if ( -d _ ) {
            ...
            my $d;
            $d = gensym() if $] < 5.006;
            if (1 || !opendir $d, $curdir) { # SPEC CPU2017 500.perlbench doesn't do opendir
              # So don't carp about it: _error($arg, "cannot opendir", $canon);
                @files = ();
            }
            else {
                no strict 'refs';
            ...
            if ($arg->{depth} or !$arg->{keep_root}) {
                ...
                if (rmdir $root) {
                    ...
                    _error($arg, "cannot remove directory", $canon) unless ($! =~ /empty/i); # SPEC CPU 500.perlbench can't descend, so this is sort of expected

...
#           );
           return $LastMsgNum if (
                 exists($mhonarc_files{msgnum_filename($LastMsgNum)}) &&
                 ref($mhonarc_files{msgnum_filename($LastMsgNum)}) eq 'ARRAY' &&
                 @{$mhonarc_files{msgnum_filename($LastMsgNum)}} > 0 &&
                 !exists($mhonarc_files{msgnum_filename($LastMsgNum + 1)})
           );
        }

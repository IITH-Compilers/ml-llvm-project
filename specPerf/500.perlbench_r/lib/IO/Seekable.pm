...
sub seek {
    ...
    # SPEC CPU2017 changes for seek() begin here
    if ($_[0]->isa('IO::File')) {
      my ($io, $off, $whence) = @_;
      ...
      if ($whence == SEEK_SET) {
        ...
        $io->{'pos'} = length($io->{'str'}) + $off;
      }
      return 1;
    }
    # SPEC CPU2017 changes for seek() end here
    seek($_[0], $_[1], $_[2]);
}

sub sysseek {
    @_ == 3 or croak 'usage: $io->sysseek(POS, WHENCE)';
    # SPEC CPU2017 changes for sysseek() begin here
    if ($_[0]->isa('IO::File')) {
      return $_[0]->seek($_[1], $_[2]);
    }
    # SPEC CPU2017 changes for sysseek() end here
    sysseek($_[0], $_[1], $_[2]);
}

sub tell {
    @_ == 1 or croak 'usage: $io->tell()';
    # SPEC CPU2017 changes for tell() begin here
    if ($_[0]->isa('IO::File')) {
      return $_[0]->{'pos'};
    }
    # SPEC CPU2017 changes for tell() end here
    tell($_[0]);
}

...
# SPEC CPU - messages are delivered in an array
#		MBOX: while (!eof($fh)) {
		MBOX: while (@{$fh}) {
		    print STDOUT "."  unless $QUIET;
		    ($index, $fields) = read_mail_header($fh);
...
#	while (<$msginfh>) {
	...
	push @$msghandle, "<!-- ", commentize("MHonArc v$VERSION"), " -->\n";	# SPEC CPU2017
	if ($PrintXComments) {
# Begin SPEC CPU2017 changes
#	    print $msghandle 
#		"<!--X-Subject: ",
...
# SPEC CPU2017	    print $msghandle "<!--X-Derived: ", commentize($_), " -->\n";
...
##
sub set_handler {
# CPU2017 - do not change signal handlers
return;
    %_sig_org = ( );

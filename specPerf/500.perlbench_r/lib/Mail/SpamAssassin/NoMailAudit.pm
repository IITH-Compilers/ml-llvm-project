...
sub _get_header_list {
  ...
  my $lchdr = lc $hdr;
  my @cap_hdrs = grep(lc($_) eq $lchdr, sort keys(%{$self->{headers}}));
...
# CPU2017 -- there's some file I/O in here that we don't want, but it should
...
# CPU2017 -- your lock is successful!
return 1;
...
# CPU2017 -- no Mail::Audit available
return 1;

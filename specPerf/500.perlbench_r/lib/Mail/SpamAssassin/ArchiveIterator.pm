...
#  # There is no socket IO or child processes in CPU2017
#  elsif ($self->{opt_j} > 1) {
...
## There are no children in CPU2017
#sub start_children {
...
sub mail_open {
  ...
  my $expr;
  # If at all possible, we avoid fork()ing in CPU2017
...
sub fix_globs {
  ...
  #$path =~ s/^~/$ENV{'HOME'}/;
  # In CPU2017, the home directory is always '.'
  $path =~ s/^~/./;

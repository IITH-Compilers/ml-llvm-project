...
use Config;
# CPU2017 -- doesn't have File::Spec
#use File::Spec;
use Time::Local;
# CPU2017 -- there is only one hostname, and it is ours
...
# CPU2017 -- never called, but just in case
return undef;
...
# CPU2017 -- not running in taint mode...
...
# CPU2017 -- doesn't run in taint mode
return 0;
...
sub time_to_rfc822_date {
  ...
  # CPU2017 is always GMT, and never the current time
...
# CPU2017 -- all users are the same
...
# CPU2017 -- there's only one hostname
$hostname = 'perlbench';

...
sub _check_whitelist_rcvd {
  ...
  $addr = lc $addr;
  foreach my $white_addr (sort keys %{$list}) {
...
# CPU2017 -- yes, all mail is recent
return 1;
...
# CPU2017 - do not check current time
...
# CPU2017 -- do not let the local timezone taint even the debug messages
...
sub _check_attachments {
  ...
  foreach my $str (sort keys %state) {
    if ($state{$str} != 0) {

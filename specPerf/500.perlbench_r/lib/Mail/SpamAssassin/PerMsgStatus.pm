...
sub check {
  ...
  # In CPU2017 let's truncate them instead, in order to avoid rounding issues
...
sub rewrite_as_spam {
  ...
  foreach my $header (sort keys %{$self->{conf}->{headers_spam}} ) {
...
# CPU2017 -- now is never the time. :)
#	"\t" . Mail::SpamAssassin::Util::time_to_rfc822_date() . "\n" .
	"\t" . Mail::SpamAssassin::Util::time_to_rfc822_date(83273400) . "\n" .
	    $newmsg;

  # MIME boundary
# CPU2017 -- fix this so that folks will be able to validate in the future
...
sub rewrite_headers {
  ...
  if($self->{is_spam}) {
    ...
    foreach my $header (sort keys %{$self->{conf}->{headers_ham}} ) {
...
sub _get_tag {
  ...
  %tags = ( YESNOCAPS => sub { $self->{is_spam} ? "YES" : "NO"; },
	    ...
	    YESNO => sub { $self->{is_spam} ? "Yes" : "No"; },
            ...
            # CPU2017 changes here -- I forgot to save the previous lines
	    ...
	    DATE => sub {
# CPU2017 -- make this constant, too
#	      Mail::SpamAssassin::Util::time_to_rfc822_date();
...
sub do_meta_tests {
  ...
  my @metas = sort keys %{ $self->{conf}{meta_tests} };
  ...
  $evalstr .= join("\n", (map { "\$self->{'tests_already_hit'}->{'$_'} ||= 0;" } sort keys %setup_rules), "");
...
# CPU2017 -- this shouldn't get called, but just in case...
return undef;

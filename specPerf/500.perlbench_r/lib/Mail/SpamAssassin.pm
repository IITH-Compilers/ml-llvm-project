...
BEGIN {
# CPU2017 -- Time::HiRes is not available
...
$HOME_URL = "http://www.spec.org/cpu2017/";
...
# CPU2017 -- no locking; it's all serial
...
sub trim_rules {
  ...
  foreach my $rule_type (@rule_types) {
    push(@all_rules, sort keys(%{$self->{conf}->{$rule_type}}));
  }
  ...
  my @meta_tests    = grep(/$regexp/, sort keys(%{$self->{conf}->{meta_tests}}));
  ...
  foreach my $rule_type (@rule_types) {
    foreach my $rule (sort keys(%{$self->{conf}->{$rule_type}})) {
...
sub report_as_spam {
  ...
  # Reporting is disabled for use with CPU2017
...
sub reply_with_warning {
  ...
  # Replies are disabled for CPU2017
...
# CPU2017 -- do not let time slip past us even in this place where it will
...
sub get_and_create_userstate_dir {
  ...
  if (defined $self->{user_dir}) {
    # Changed to non-hidden for CPU2017
...
# For CPU2017, the home directory is always '.'
return '.';
...
# CPU2017 -- hack it because there's no opendir
my @cfs = ();
if ($dir eq './rules') {
@cfs = qw(10_misc.cf
20_anti_ratware.cf
...
sub encapsulate_mail_object {
  ...
  # For CPU2017, there is no "my_mail_class" :)

...
bool RDGWrapperPass::runOnFunction(Function &F){
	...
	for(LoopInfo::iterator i = LI.begin(), e = LI.end(); i != e; ++i){
		...
		auto &currContext = L->getHeader()->getParent()->getParent()->getContext();
		MDBuilder loopMDBuilder(currContext);

		llvm::SmallVector<llvm::Metadata *, 2> loopIDValues;

		// create loop metadata node with custom id
		auto curID = loopNum;
		loopIDValues.push_back(loopMDBuilder.createString(std::to_string(loopNum)));
		auto *intType = llvm::Type::getInt32Ty(currContext);

		loopIDValues.push_back(
      loopMDBuilder.createConstant(llvm::ConstantInt::get(intType, curID)));

	  auto *const loopIdMD = llvm::MDNode::get(currContext, loopIDValues);
...
// place loop id first
  auto newLoopIdMD = llvm::MDNode::get(currContext, newLoopMDs);
  newLoopIdMD->replaceOperandWith(0, newLoopIdMD);

  L->setLoopID(newLoopIdMD);

		// L->getLoopID();

		errs() << "Get Loop ID: " << *(L->getLoopID()) << "\n";
		// errs() << & << "\n";
		...
		Print_IR2Vec_File(G1, Filename2, DDist, instVecMap);
	}
	return false;

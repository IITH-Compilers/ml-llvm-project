...
static cl::opt<unsigned>
    HugeSizeForSplit("ml-huge-size-for-split", cl::Hidden,
...
template <> struct MappingTraits<Color2Reg> {
  static void mapping(IO &io, Color2Reg &info) {
    io.mapRequired("color", info.color);
    io.mapOptional("phyReg", info.phyReg);
  }
  static const bool flow = true;
};

// template <> struct SequenceTraits<std::vector<Color2Reg>> {
//   static size_t size(IO &io, std::vector<Color2Reg> &list) {
//     return list.size();
//   }
//   static Color2Reg &element(IO &io, std::vector<Color2Reg> &list,
//                             size_t index) {
//     return list[index];
//   }
//   // static const bool flow = true;
...
/*    template <> struct DocumentListTraits<std::vector<Color2Reg>> {
          static size_t size(IO &io, std::vector<Color2Reg> &list) {
                  return list.size();
          }
          static Color2Reg & element(IO &io, std::vector<Color2Reg> &list,
size_t index) { return list[index];
                    }
          static const bool flow = true;
};*/
...
char MLRA::ID = 0;
// char &llvm::MLRAID = MLRA::ID;

INITIALIZE_PASS_BEGIN(MLRA, "mlra", "ML Register Allocator", false, false)
...
INITIALIZE_PASS_END(MLRA, "mlra", "ML Register Allocator", false, false)

#ifndef NDEBUG
const char *const MLRA::StageName[] = {"RS_New",    "RS_Assign", "RS_Split",
                                       "RS_Split2", "RS_Spill",  "RS_Memory",
                                       "RS_Done"};
#endif
...
FunctionPass *llvm::createMLRegisterAllocator() { return new MLRA(); }

FunctionPass *
llvm::createMLRegisterAllocator(DenseMap<unsigned, unsigned> VirtRegToColor) {
  return new MLRA(VirtRegToColor);
}

static RegisterRegAlloc
    mlraRegAlloc("mlra", "machine learning based register allocator",
                 createMLRegisterAllocator);

// struct RegisterColors {
//   unsigned color;
//   unsigned phyReg;
// };
...
// // namespace yaml {
// // namespace llvm {
// template <> struct llvm::yaml::MappingTraits<RegisterColors> {
//   static void mapping(IO &io, RegisterColors &info) {
//     io.mapRequired("color", info.color);
//     io.mapRequired("phyReg", info.phyReg);
//   }
// };
// // } // namespace llvm
// // } // namespace yaml

// typedef std::vector<RegisterColors> RegisterColorList;
// std::vector<RegisterColorList> RegisterColor;
...
#if 0
  // auto Buffer = llvm::MemoryBuffer::getFile(config_colorMap);
  // if (!Buffer) {
  //   llvm::errs() << "Can't open " << config_colorMap << "\n";
  //   return;
  // }

  // Input yin(Buffer.get()->getBuffer());
  // yin >> RegisterColor;

  // if (yin.error()) {
  //   errs() << "Error .. yam \n";
  // } else {
  //   errs() << "No error occured \n";
  // }

  // // Process read document
  // for (RegisterColorList &rc : RegisterColor) {
  //   for (RegisterColors &reg : rc) {
  //     errs() << "color =" << reg.color << "\n";
  //     errs() << "phyReg =" << reg.phyReg << "\n";
  //   }
  // }
  // llvm_unreachable("Exiting test");

  LLVM_DEBUG(errs() << config_colorMap << "\n");
  std::ifstream configFile(config_colorMap);
  std::string yamlString;
  yamlString.assign((std::istreambuf_iterator<char>(configFile)),
                    (std::istreambuf_iterator<char>()));

  Input yin(yamlString);

  if (yin.error()) {
    errs() << "Erroor .. yam \n";
  } else {
    errs() << "No error occupres \n";
  }
  // errs () << "yamlString " << yamlString;
  // LLVM_YAML_IS_DOCUMENT_LIST_VECTOR(llvm::yaml::Color2Reg);
  // Parse the YAML file
  typedef std::vector<llvm::yaml::Color2Reg> Color2RegList;
  Color2RegList docs;
  // MyDocType theDoc;
  // i yin >> theDoc;
  yin >> docs; // Failing
...
bool MLRA::LRE_CanEraseVirtReg(unsigned VirtReg) {
...
void MLRA::LRE_WillShrinkVirtReg(unsigned VirtReg) {
  if (!VRM->hasPhys(VirtReg))
    return;
...
void MLRA::LRE_DidCloneVirtReg(unsigned New, unsigned Old) {
...
void MLRA::releaseMemory() {
  SpillerInstance.reset();
  ExtraRegInfo.clear();
  GlobalCand.clear();
}

void MLRA::enqueue(LiveInterval *LI) { enqueue(Queue, LI); }

void MLRA::enqueue(PQueue &CurQueue, LiveInterval *LI) {
...
LiveInterval *MLRA::dequeue() { return dequeue(Queue); }

LiveInterval *MLRA::dequeue(PQueue &CurQueue) {
  if (CurQueue.empty())
    return nullptr;
...
unsigned MLRA::tryAssign(LiveInterval &VirtReg, AllocationOrder &Order,
                         SmallVectorImpl<unsigned> &NewVRegs,
                         const SmallVirtRegSet &FixedRegisters) {
  Order.rewind();
  unsigned PhysReg;
...
unsigned MLRA::canReassign(LiveInterval &VirtReg, unsigned PrevReg) {
...
bool MLRA::shouldEvict(LiveInterval &A, bool IsHint, LiveInterval &B,
                       bool BreaksHint) {
...
bool MLRA::canEvictInterference(LiveInterval &VirtReg, unsigned PhysReg,
...
bool MLRA::canEvictInterferenceInRange(LiveInterval &VirtReg, unsigned PhysReg,
                                       SlotIndex Start, SlotIndex End,
                                       EvictionCost &MaxCost) {
  EvictionCost Cost;
...
unsigned MLRA::getCheapestEvicteeWeight(const AllocationOrder &Order,
                                        LiveInterval &VirtReg, SlotIndex Start,
                                        SlotIndex End, float *BestEvictweight) {
...
/// returned true.
void MLRA::evictInterference(LiveInterval &VirtReg, unsigned PhysReg,
                             SmallVectorImpl<unsigned> &NewVRegs) {
...
bool MLRA::isUnusedCalleeSavedReg(unsigned PhysReg) const {
...
unsigned MLRA::tryEvict(LiveInterval &VirtReg, AllocationOrder &Order,
                        SmallVectorImpl<unsigned> &NewVRegs,
...
bool MLRA::addSplitConstraints(InterferenceCache::Cursor Intf,
                               BlockFrequency &Cost) {
...
bool MLRA::addThroughConstraints(InterferenceCache::Cursor Intf,
...
bool MLRA::growRegion(GlobalSplitCandidate &Cand) {
...
bool MLRA::calcCompactRegion(GlobalSplitCandidate &Cand) {
...
BlockFrequency MLRA::calcSpillCost() {
...
/// otherwise.
bool MLRA::splitCanCauseEvictionChain(unsigned Evictee,
...
/// otherwise.
bool MLRA::splitCanCauseLocalSpill(unsigned VirtRegToSplit,
                                   GlobalSplitCandidate &Cand,
                                   unsigned BBNumber,
                                   const AllocationOrder &Order) {
  Cand.Intf.moveToBlock(BBNumber);
...
///
BlockFrequency MLRA::calcGlobalSplitCost(GlobalSplitCandidate &Cand,
                                         const AllocationOrder &Order,
                                         bool *CanCauseEvictionChain) {
...
void MLRA::splitAroundRegion(LiveRangeEdit &LREdit,
...
// worth the cost.
unsigned MLRA::isSplitBenefitWorthCost(LiveInterval &VirtReg) {
...
unsigned MLRA::tryRegionSplit(LiveInterval &VirtReg, AllocationOrder &Order,
                              SmallVectorImpl<unsigned> &NewVRegs) {
...
unsigned MLRA::calculateRegionSplitCost(LiveInterval &VirtReg,
                                        AllocationOrder &Order,
                                        BlockFrequency &BestCost,
                                        unsigned &NumCands, bool IgnoreCSR,
                                        bool *CanCauseEvictionChain) {
...
unsigned MLRA::doRegionSplit(LiveInterval &VirtReg, unsigned BestCand,
                             bool HasCompact,
                             SmallVectorImpl<unsigned> &NewVRegs) {
...
/// they don't allocate.
unsigned MLRA::tryBlockSplit(LiveInterval &VirtReg, AllocationOrder &Order,
                             SmallVectorImpl<unsigned> &NewVRegs) {
...
unsigned MLRA::tryInstructionSplit(LiveInterval &VirtReg,
                                   AllocationOrder &Order,
                                   SmallVectorImpl<unsigned> &NewVRegs) {
...
///
void MLRA::calcGapWeights(unsigned PhysReg, SmallVectorImpl<float> &GapWeight) {
...
/// basic block.
///
unsigned MLRA::tryLocalSplit(LiveInterval &VirtReg, AllocationOrder &Order,
                             SmallVectorImpl<unsigned> &NewVRegs) {
...
unsigned MLRA::trySplit(LiveInterval &VirtReg, AllocationOrder &Order,
                        SmallVectorImpl<unsigned> &NewVRegs,
                        const SmallVirtRegSet &FixedRegisters) {
...
/// recolored.
bool MLRA::mayRecolorAllInterferences(unsigned PhysReg, LiveInterval &VirtReg,
                                      SmallLISet &RecoloringCandidates,
                                      const SmallVirtRegSet &FixedRegisters) {
  const TargetRegisterClass *CurRC = MRI->getRegClass(VirtReg.reg);
...
/// exists.
unsigned MLRA::tryLastChanceRecoloring(LiveInterval &VirtReg,
                                       AllocationOrder &Order,
                                       SmallVectorImpl<unsigned> &NewVRegs,
...
bool MLRA::tryRecoloringCandidates(PQueue &RecoloringQueue,
                                   SmallVectorImpl<unsigned> &NewVRegs,
                                   SmallVirtRegSet &FixedRegisters,
                                   unsigned Depth) {
...
unsigned MLRA::selectOrSplit(LiveInterval &VirtReg,
                             SmallVectorImpl<unsigned> &NewVRegs) {
  CutOffInfo = CO_None;
...
unsigned MLRA::tryAssignCSRFirstTime(LiveInterval &VirtReg,
...
void MLRA::aboutToRemoveInterval(LiveInterval &LI) {
  // Do not keep invalid information around.
  SetOfBrokenHints.remove(&LI);
}

void MLRA::initializeCSRCost() {
...
void MLRA::collectHintInfo(unsigned Reg, HintsInfo &Out) {
...
BlockFrequency MLRA::getBrokenHintFreq(const HintsInfo &List,
                                       unsigned PhysReg) {
  BlockFrequency Cost = 0;
...
/// and new register.
void MLRA::tryHintRecoloring(LiveInterval &VirtReg) {
...
unsigned MLRA::selectOrSplitImpl(LiveInterval &VirtReg,
                                 SmallVectorImpl<unsigned> &NewVRegs,
                                 SmallVirtRegSet &FixedRegisters,
                                 unsigned Depth) {
...
void MLRA::reportNumberOfSplillsReloads(MachineLoop *L, unsigned &Reloads,
...
bool MLRA::spillInterferences(LiveInterval &VirtReg, unsigned PhysReg,
                              SmallVectorImpl<unsigned> &SplitVRegs) {
...
void MLRA::allocatePhysRegsViaRL() {
  ...
  while (LiveInterval *VirtReg = dequeue()) {
    ...
    LLVM_DEBUG(errs() << "Register class(" << regClass
                      << ") is not supported by MLRA so calling Greedy.\n");

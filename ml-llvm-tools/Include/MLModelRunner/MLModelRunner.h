...
#define setFeature(TYPE)                                                       \
  virtual void setFeature(const std::string &name, const TYPE &value) {        \
    Serializer->setFeature(name, value);                                       \
  }
  SUPPORTED_TYPES(setFeature)
#undef setFeature
...
protected:
  MLModelRunner(LLVMContext &Ctx, Kind Type,
    ...
    case BaseSerializer::Kind::Bitstream:
      ...
      break;
    case BaseSerializer::Kind::Tensorflow:
      errs() << "In Tensorflow case...\n";
      Serializer = std::make_unique<TensorflowSerializer>();
      break;
    }

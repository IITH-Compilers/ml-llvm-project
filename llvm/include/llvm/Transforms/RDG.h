...
class RDG{
  private:
	...
	AAResults &AA; ScalarEvolution &SE; 
	LoopInfo &LI; DependenceInfo &DI;
	LoopAccessLegacyAnalysis &LAA;
  public:
	static char ID;
	// RDG(AAResults &AA, ScalarEvolution &SE, LoopInfo &LI, DependenceInfo &DI, LoopAccessAnalysis& LAA):AA(AA),SE(SE),LI(LI),DI(DI),LAA(LAA){
	// }
	RDG(AAResults &AA, ScalarEvolution &SE, LoopInfo &LI, DependenceInfo &DI, LoopAccessLegacyAnalysis& LAA):AA(AA),SE(SE),LI(LI),DI(DI),LAA(LAA){}

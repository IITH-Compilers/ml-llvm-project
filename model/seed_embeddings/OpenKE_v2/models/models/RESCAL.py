...
class RESCAL(Model):
	...
	def embedding_def(self):
		...
		self.ent_embeddings = tf.keras.layers.Embedding(config.entTotal, config.hidden_size, name="ent_embeddings", embeddings_initializer=tf.compat.v1.keras.initializers.he_uniform())
		self.rel_matrices = tf.keras.layers.Embedding(config.relTotal, config.hidden_size * config.hidden_size, name="rel_matrices", embeddings_initializer=tf.compat.v1.keras.initializers.he_uniform())
	...
	def loss_def(self):
		...
		self.loss = tf.reduce_mean(input_tensor=tf.maximum(n_score - p_score + config.margin, 0))

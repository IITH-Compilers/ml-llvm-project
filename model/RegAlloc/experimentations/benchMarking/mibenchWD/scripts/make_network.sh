BENCHMARK="/home/cs18mtech11030/project/grpc_llvm/ML-Register-Allocation/model/RegAlloc/experimentations/benchMarking/mibenchWD/mibench"
LLVM_BUILD="/home/cs18mtech11030/project/grpc_llvm/ML-Register-Allocation/model/RegAlloc/x86_release_build"
# LLVM_BUILD="/usr"
REGALLOC_TYPE=$1
if [ -z ${REGALLOC_TYPE} ];
then
	echo "Enter the register allocator .."
	exit
fi
...
time ./dijkstra_small.out input.dat > output_small_${REGALLOC_TYPE}_${ENABLE_ML_REGALLOC}.dat


######### network: patricia

cd ${BENCHMARK}/network/patricia
...
time ./patricia_final.out large.udp > output_large_${REGALLOC_TYPE}_${ENABLE_ML_REGALLOC}.txt
time ./patricia_final.out small.udp > output_small_${REGALLOC_TYPE}_${ENABLE_ML_REGALLOC}.txt

############ automotive: basicmath

cd ${BENCHMARK}/automotive/basicmath
...
time ./automotive_small_final.out > output_small_${REGALLOC_TYPE}_${ENABLE_ML_REGALLOC}.txt
...
time ./automotive_large_final.out > output_large_${REGALLOC_TYPE}_${ENABLE_ML_REGALLOC}.txt

########### automotive: bitcount

cd ${BENCHMARK}/automotive/bitcount
...
time ./bitcount.out 75000 > output_small_${REGALLOC_TYPE}_${ENABLE_ML_REGALLOC}.txt
time ./bitcount.out 1125000 > output_large_${REGALLOC_TYPE}_${ENABLE_ML_REGALLOC}.txt


// automotive: qsort

cd ${BENCHMARK}/automotive/qsort
...
time ./qsort_large.out input_large.dat > output_large_${REGALLOC_TYPE}_${ENABLE_ML_REGALLOC}.txt

// automotive: susan

cd ${BENCHMARK}/automotive/susan
...
time ./susan.out input_small.pgm output_small.smoothing.pgm -s
time ./susan.out input_small.pgm output_small.edges.pgm -e
time ./susan.out input_small.pgm output_small.corners.pgm -c


time ./susan.out input_large.pgm output_large.smoothing.pgm -s
time ./susan.out input_large.pgm output_large.edges.pgm -e
time ./susan.out input_large.pgm output_large.corners.pgm -c

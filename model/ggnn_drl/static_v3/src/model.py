...
class QNetwork(nn.Module):
    ...
    def __init__(self, state_size, action_size, seed, fc1_units=64, fc2_units=64):
        ...
        self.inlineNet = InlineNetwork(state_size, seed)
    ...
    def forward(self, state,isStart=False):
        ...
        inline_state, distribution_state = state
        
        if inline_state is  not None:
            return self.inlineNet(inline_state) 

        out = self.transitionNet(distribution_state)
        # indexchoose = torch.argmax(out)
        # Trans_Qvalue = torch.max(out)
        
        out2=None
        if not isStart:
            out2 = self.distributeNet(distribution_state)

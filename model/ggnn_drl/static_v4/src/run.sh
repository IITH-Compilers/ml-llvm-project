...
then 
        ...
        TRAINED_MODEL=${MODEL_PAR}/${DATA_SET_NAME}/ELC_${SET_ELC}_AMF_${SET_AMF}_${BUILD_TYPE}_PDP_${POST_DIS_PASSES_ARG}_EP${EPOCHS}_${KEY_POINT}/${MODE}
...
echo "Logs files: ${LOG}"
## Call the py script 
python  main.py --script ${PY_SPT} --dataset=${DATA_SET} ${AMF} ${ELC} ${DISABLE_EXEC_BIN} ${PDP} ${RT} --trained_model=${TRAINED_MODEL} --distributed_data=${DIST_GEN_DATA}  --logdir ${LOG} --mode ${MODE_PROCESS} --epochs ${EPOCHS}

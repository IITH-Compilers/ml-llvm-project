...
def call_distributionPass(filename, distributeSeq, method_name, fun_id, loop_id, distributed_data):
    

    try:
        parts = os.path.split(filename)
        if config.mode == 'train':
            dist_llfile = "Distribute_{filename}_F{fun_id}_L{loop_id}.ll".format(filename=parts[1], method_name=method_name, loop_id=loop_id,fun_id=fun_id)
        else:    
...
def get_parse_args():
    ...
    parser.add_argument('--state_size', dest='state_size', type=int, required=False, help='Size of the hidden input vector for the state.', default=5)

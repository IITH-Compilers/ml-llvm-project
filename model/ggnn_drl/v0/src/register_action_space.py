...
class RegisterActionSpace:
    ...
    def __init__(self, target):
        # :%s/\(.*"color":\).*\(,\n.*\n.*"phyReg":\)\(.*\)/\1\3\2\3/g
        self.spill = 0
        self.supported_regclasses, self.suppcls_regs_map, self.reg_idname_map = RegisterActionSpace.loadRegConfig(target)
        ...
        # list()
        supp_regs_normal_values_map = list(map(lambda y: list(map(lambda reg: org_normal_map[reg],  y)), self.suppcls_regs_map.values()))
...
#         return action_space
    ...
    def maskActionSpace(self, regClass, adj_colors):
        if regClass in self.supported_regclasses:
            ...
            if len(adj_colors) > 0:
                ...
                action_space = list(filter(lambda x: x not in adj_colors, action_space)) # np.delete(action_space, adj_colors)
        else:
            logging.warning('RegClass not present in the map = {}'.format(regClass))
    ...
    def loadRegConfig(target):
        ...
        return supported_regClass, suppcls_regs_map, reg_idname_map 

...
from singleagentEnv_v1 import HierarchicalGraphColorEnv
from register_action_space import RegisterActionSpace
from ray.rllib.models import ModelCatalog
from model_singleagent_v1 import ColorNetwork, SplitNetwork
import logging
...
def experiment(config):
    ...
    for i in range(iterations):
        ...
        if i == iterations - 1 or (train_results['episodes_total'] - last_checkpoint) > 500:
    ...
    if config['dump_onnx_model']:   
        train_agent.export_policy_model(export_dir=MODEL_DIR + "/color_agent", policy_id="color_agent_model", onnx=11)
        train_agent.export_policy_model(export_dir=MODEL_DIR + "/split_agent", policy_id="split_agent_model", onnx=11)
...
if __name__ == "__main__":
    ...
    ModelCatalog.register_custom_model("color_agent_model", ColorNetwork)
    ModelCatalog.register_custom_model("split_agent_model", SplitNetwork)

    max_edge_count = config["env_config"]["max_edge_count"]

    def policy_mapping_fn(agent_id, episode=None, **kwargs):
        if agent_id.startswith("color_node_agent"):
            return "color_node_policy"
        elif agent_id.startswith("split_node_agent"):
            return "split_node_policy"
    
    adjacency_lists = Dict({
            "node_num": Discrete(config["env_config"]["max_number_nodes"]),
            "edge_num": Discrete(config["env_config"]["max_edge_count"]),
            "data": Repeated(Box(0.0, config["env_config"]["max_number_nodes"], shape=(2,), dtype=np.float32), max_len = config["env_config"]["max_edge_count"])
    })
    
    graph_obs = Box(FLOAT_MIN, FLOAT_MAX, shape=(config["env_config"]["max_number_nodes"], config["env_config"]["state_size"]), dtype=np.float32)
    node_obs = Box(FLOAT_MIN, FLOAT_MAX, shape=(config["env_config"]["state_size"], ), dtype=np.float32)
    ...
    policies = {
        "color_node_policy": (None, obs_color_node,
                                Discrete(config["env_config"]["max_number_nodes"]*config["env_config"]["action_space_size"]),{
                                "gamma": 0.9,
                                "model": {
                                    "custom_model": "color_agent_model",
                                    "custom_model_config": {

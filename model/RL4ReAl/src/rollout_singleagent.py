...
class RollOutInference:
    def __init__(self,  args):
        ...
        if args.dump_onnx_model:
            agent.export_policy_model(export_dir=MODEL_DIR + "/single_agent", policy_id="single_agent_model", onnx=11)
        # agent.stop()
    ...
    def compute_action(self):
        ...
        actions_response = {}
        while not done:        
            multi_obs = self.obs if self.multiagent else {_DUMMY_AGENT_ID: self.obs}
            action_dict = {}
        ...
        actions_response["color_map"] = self.env.colormap
            

        return actions_response, 0

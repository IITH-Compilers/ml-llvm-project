import sys, os
from config import BUILD_DIR, MODEL_PATH, MODEL_DIR, REPO_DIR
sys.path.append(
...
import rollout_singleagent as inference
...
class service_server(
    ...
    def getAdvice(self, request, context):
        try:
            ...
            if self.inference_model.getLastTaskDone() == 0:
                print('Entered color...')
                out = {
                    "action": "Color",
                    "color": action["color_map"],
...
def run_pipe_communication(data_format, pipe_name, dump_onnx_model=False):
    ...
    while True:
        try:
            ...
            if inference_model.getLastTaskDone() == 0:

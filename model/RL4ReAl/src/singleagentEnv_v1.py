...
class HierarchicalGraphColorEnv(MultiAgentEnv):
    ...
    def reset(self, graph=None):
        ...
        self.color_node_agent_id = "color_node_agent_{}".format(self.agent_count)
    ...
    def _color_step(self, action):
        ...
        # Coloring the node
        colour_reward, done_all_coloring, response  = self.step_colorTask(colour_action)
        # print("Colour_reward: ", colour_reward)
        ...
        select_node_mask = self.createNodeSelectMask()
        # select_node_mask = self.createNodeSelectMaskSpillWeightBased()
        if all(v == 0 for v in select_node_mask) and not done_all_coloring:
            done_all_coloring = True
        ...
        if done_all_coloring:
            ...
            c_reward = colour_reward
            c_done = done_all_coloring
        
            obs[self.color_node_agent_id] = c_obs
        
        if s_obs is not None:
            obs[self.split_node_agent_id] = s_obs
            
        reward = {
            self.color_node_agent_id: c_reward,
            self.split_node_agent_id: s_reward
        }
        
        done = {
            self.color_node_agent_id: c_done,
            self.split_node_agent_id: s_done,
            "__all__": done_all
        }
        
        if done_all:
            os.killpg(os.getpgid(self.server_pid.pid), signal.SIGKILL)
        logging.debug("Exit _colour_step")
        return obs, reward, done, {}
    
    def _split_step(self, action):
        logging.debug("Enter _split_step")
        
        print("Entered splitting step")
        
        split_map = []
            
        for i, cur_node in enumerate(self.uncolored_nodes):
            use_distances = self.obs.use_distances[cur_node]
            done = False
            logging.debug("****Split index****** {} {}".format( len(use_distances), use_distances))
            userDistanceDiff = 0
            
            split_index = action[i]
            split_point = split_index
            use_distance_list = self.obs.use_distances[cur_node]
            
            if split_point != len(use_distance_list)-1:
                # print(f"Splitting node {cur_node} at {split_point}")
                split_map.append(RegisterAllocation_pb2.Data.splitData(regidx=int(self.obs.idx_nid[cur_node]), splitpt=int(split_point)))
        ...
        # Handling mask all zero issue
        if all(v==0 for v in select_node_mask):
            split_done = True
        ...
        if self.mode == 'inference':
            ...
            reward[self.color_node_agent_id] = 0
        ...
        logging.debug("Exit _split_step")
        return obs, reward, done, {}
    ...
    def step(self, action_dict, extra_info=None):
        if self.mode != 'inference':
            if extra_info and 'color_node_policy' in extra_info.keys():
                self.node_representation_mat = extra_info['color_node_policy'][0, :, :]                
        else:
            if extra_info is not None:
                if len(extra_info.shape) == 3:
                    self.node_representation_mat = extra_info[0, :, :]                
        
        if self.color_node_agent_id in action_dict:
            return self._color_step(action_dict[self.color_node_agent_id])
        
        if self.split_node_agent_id in action_dict:
            return self._split_step(action_dict[self.split_node_agent_id])
    ...
    def getUsepointProperties(self, node):
        psw = self.obs.positionalSpillWeights[node]
        use_distances = self.obs.use_distances[node]
    ...
    def step_splitTask(self, split_map):
        ...
        if self.mode != 'inference':
            ...
            # if update obs result is false that means all the splits are failed so terminating the episode       
    ...
    def step_colorTask(self, action):
        ...
        if False not in self.obs.graph_topology.discovered:
            ...
            # self.obs.next_stage = 'end'
            if self.mode != 'inference':
                exit_response = self.stable_grpc('Exit', split=None)
    ...
    def reset_env(self, graph=None):
        ...
        if self.mode != 'inference':
            ...
            self.spilled = [False]*self.num_nodes
        ...
        while True:
            try:
                ...
                if op == "Split":
                    ...
                    updated_graphs = self.queryllvm.codeGen(op, split=split, color=self.colormap)
                t2 = time.time()
                self.grpc_rtt += t2-t1
                break
    ...
    def update_obs(self, updated_graphs, split_map):
        def sc(vec, sw):
                vec[-1] = sw
                return vec
        
        if any(updated_graphs.result):
            num_edges = 0
            new_node_count = 0
            ...
            for idx,split in enumerate(split_map):
                register_id = split.regidx
                split_point = split.splitpt
                split_result = updated_graphs.result[idx]
        ...
        return any(updated_graphs.result)

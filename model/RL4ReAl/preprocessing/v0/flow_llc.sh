...
HOME=/home/cs20mtech12003/RL4ReAl-2.0
...
INP_DIR=/Pramana/RL4Real/Training-data/profiled-data/spec17
...
declare -A MODELS=([mlra]=" -regalloc=greedy -mlra-dump-ig-dot -ml-config-path=/home/cs20mtech12003/RL4ReAl-2.0/config " [mlbasicra]=" -mllvm -regalloc=basic  -mllvm -mlra-dump-ig-dot ")
...
WD=${SRCH_FLR}_${MODE}_${MODEL}_${TARGET}_prof_data_01_08_24
...
then   
 ...
 for d in ${INP_DIR}/${IP_FLR_NAME}/*.ll; do 
          # generate "$d" " " &
      name=`basename ${d}` && oname=${name%.*} && rfile= && if [ ! -z "${REMARKS}" ]; then rfile="-pass-remarks-output=${oname}_${G_TYPE}.yaml"; fi &&  cd ${IG_DOT} && ${TIME_OUT} ${LLVM_BUILD}/bin/llc ${INCLUDE} ${OPT_PASSES_SEQ} ${MODEL_ARGS} ${USE_MCA} ${REMARKS} ${rfile}  ${d} -o /dev/null && ${LLVM_BUILD}/bin/llc ${INCLUDE} ${OPT_PASSES_SEQ} -S  ${d} -o ${ASM_DIR}/${oname}.s &&  ${LLVM_BUILD}/bin/clang ${INCLUDE} ${ASM_DIR}/${oname}.s -o ${BIN_DIR}/${oname}.out &
          # pids[${i}]=$!
 done 
# wait for all pids
    for pid in ${pids[*]}; do

...
def get_O3_runtimes(rundir, isInputRequired):
    ...
    try:
        print(
            'Checking if local O3_runtimes.pkl file exists to avoid waste of compilation.')
        with open(os.path.join(rundir, 'O3_runtimes.pkl'), 'rb') as f:
            print('returning preprocess O3 runtimes')
            return pk.load(f)
    except:
        print('Did not find O3_runtimes.pkl...',
              'Compiling to get -O3 runtimes.')
        pass
    ...
    llfiles_validjson = [os.path.join(
        O3_folder, getllfileNameFromJSON(json)) for json in jsons]
    ...
    if isInputRequired:
        ...
        assert len(input_files) == len(
            llfiles), ' Count of source and input should be same'
...
def get_output(filename, config, inputd=None, file_format='ll'):
    ...
    try:
        if file_format == 'll':
            ...
            out_file = "{part1}outfiles{part2}".format(
                part1=parts[0], part2=parts[1][:-3]+'.out')

            # if not path.exists(out_file):
            cmd1 = "timeout --kill-after=2m 2m {clang} {input_file} ../common/common.c ../common/dummy.c -I../common -o {out_file}".format(
                clang=os.environ['CLANG'], input_file=filename, out_file=out_file)
            # print(cmd1)
        ...
        if path.exists(out_file):
            if inputd is not None:
                cmd2 = "timeout --kill-after=2m 2m {out_file}<{inputd}".format(
                    out_file=out_file, inputd=inputd)
            else:
                cmd2 = "timeout --kill-after=2m 2m {out_file}".format(
                    out_file=out_file)

            # print('Runtime command\n', cmd2)
            output = subprocess.Popen(
                cmd2, executable='/bin/bash', shell=True, stdout=subprocess.PIPE).stdout.read()
        else:
        ...
        eprint('Runtime: Some error occured .. for {filename} so output={output} '.format(
            filename=filename, output=output))
    except:
        output = sys.exc_info()  # None if fails
        eprint(sys.exc_info())
        eprint('Runtime: Other Unknown Some error occured .. for {filename} so output={output} '.format(
            filename=filename, output=output))
...
def get_runtime_of_file(filename, inputd=None, file_format='ll'):
    ...
    try:
        if file_format == 'll':
            ...
            # if not path.exists(out_file):
            cmd1 = "timeout --kill-after=2m 2m {clang} {input_file} -o {out_file}".format(
                clang=os.environ['CLANG'], input_file=filename, out_file=out_file)
            # print(cmd1)
            response = os.system(cmd1)
            if response != 0:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for runtime in executor.map(execute_in_clockticks, [cmd2]*5):
                    try:
                        ...
                        raise Exception(
                            'Exception during running {}'.format(cmd2))
        ...
        eprint(sys.exc_info())
        eprint('Runtime: Exception ocurred : {}'.format(inst))
        eprint('Runtime: Some error occured .. for {filename} so runtime={runtime} '.format(
            filename=filename, runtime=runtime))
    except:
        runtime = 100000000  # None if fails
        eprint(sys.exc_info())
        eprint('Runtime: Other Unknown Some error occured .. for {filename} so runtime={runtime} '.format(
            filename=filename, runtime=runtime))
...
def distribute_and_getRuntime(filename, distributeSeq, method_name, loop_id, config, input_file_path=None):
    distributed_llfile = call_distributionPass(
        filename, distributeSeq, method_name, loop_id, config)
    Druntime = get_runtime_of_file(
        distributed_llfile, config, inputd=input_file_path)
    if Druntime == 100000000:
        eprint('Distributed file Runtime Error occured!!!!!!!!!!!!! for file={}, distributeSeq={}, method={}, loop={}'.format(
            filename, distributeSeq, method_name, loop_id))
    return Druntime
...
def distribute_and_getOutput(filename, distributeSeq, method_name, loop_id, config, input_file_path=None):
    ...
    if distributed_llfile is not None:
        ...
        output = "ll file not generated after distribution"
    return output
...
def call_distributionPass(filename, distributeSeq, method_name, loop_id, config):
    ...
    try:
        ...
        out_file = "Distribute_{distributeSeq}_{filename}L{loop_id}.ll".format(distributeSeq=distributeSeq.strip(
            ',').replace('|', 'D'), filename=parts[1], method_name=method_name, loop_id=loop_id)
        ...
        eprint('CallLoopDistribute: Some error occured while calling the distribution pass for {filename}. '.format(
            filename=filename))
        raise
    return out_file
...
def execute_in_clockticks(cmd):
    try:
        runtime = int(subprocess.Popen(cmd, executable='/bin/bash',

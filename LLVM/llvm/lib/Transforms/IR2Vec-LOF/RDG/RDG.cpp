...
// char RDG::ID = 0;
// static RegisterPass<RDG> X("RDG", "Build ReducedDependenceGraph", true, true);
...
void RDG::PrintDotFile_LAI(DataDependenceGraph &G, std::string Filename){
	...
	if(!EC){
		...
		// Appwnd all the edges into DOT File (including weights for Memory Dependence edges)
		for(auto *N : G){
...
void RDG::BuildRDG_LAI(DataDependenceGraph &G, DependenceInfo &DI, const LoopAccessInfo &LAI){
	const auto alldependences = LAI.getDepChecker().getDependences();	// List of dependences
	const SmallVector<int64_t, 8> DependenceDistances = LAI.getDepChecker().getDDist();	// List of dependence distances
...
void RDG::createMemoryEdgeMergedNode(DataDependenceGraph &G, DependenceInfo &DI, NodeType &FinalNode, NodeType &MergingNode, NodeListType &NodeDeletionList) {
        ...
        if(*tgt != FinalNode){
          ...
          if(MSL_flag == 0) {
            bool nt_flag = 0;
              ...
              if(nt_flag == 0) {
                DDGBuilder(G, DI, BBList, ReductionPHIList).createDefUseEdge(FinalNode, *tgt);
              }
          }                              
        }
          ...
          if(MSL_flag == 0) {
            bool nt_flag = 0;
              ...
              if(nt_flag == 0) {
                DDGBuilder(G, DI, BBList, ReductionPHIList).createMemoryWeightedEdge(FinalNode, *tgt, i);
              }
          }
	  }
    }
            ...
            if(MSL_flag == 0) {
              ...
              if(nt_flag == 0) {
                if(e->getKind() == EdgeKind::Rooted){
                  DDGBuilder(G, DI, BBList, ReductionPHIList).createRootedEdge(*N, FinalNode);
                } else if(e->getKind() == EdgeKind::RegisterDefUse) {
                  DDGBuilder(G, DI, BBList, ReductionPHIList).createDefUseEdge(*N, FinalNode);
                }
              }
	  ...
	  for(EdgeType *e : *N){
		  ...
		  if(*tgt == MergingNode) {
			if(e->getKind() == EdgeKind::MemoryDependence) {
				...
				if(nt_flag == 0) {
				DDGBuilder(G, DI, BBList, ReductionPHIList).createMemoryWeightedEdge(*N, FinalNode, i);
				}
			}
		  }
	  }
...
DataDependenceGraph RDG::computeRDGForInnerLoop(Loop &IL){
	...
	// 		}
			...
			// Print SCC DOT File
			// std::string SCC_Filename = "SCC_" + IL.getHeader()->getParent()->getName().str() + "_Loop" + std::to_string(loopNum) + ".dot";

...
while read p; do
	...
	while [ "$a" -lt "$NO_OF_OPT_FILES" ]; do # this is loop1
	    tmpfile=$(mktemp /tmp/IR2Vec-CollectIR.XXXXXXXXXX).ll
		opt_index=$((RANDOM % 6))
        ...
        ${LLVM_BUILD}/bin/clang -O3  -mllvm -enable-cmi -mllvm -cmi-o=$(dirname $4)/tmp/$(basename $tmpfile).cmi $tmpfile -o /dev/null &>/dev/null
		let "a++"
	    rm "$tmpfile"
	done &
	if [ $counter == 100 ];
